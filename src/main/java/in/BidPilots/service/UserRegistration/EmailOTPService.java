...
public class EmailOTPService {
    ...
    public Map<String, Object> generateAndSendOTP(String email) {
        ...
        try {
            ...
            res.put("debug_otp", otp);
        } catch (MailAuthenticationException | MailSendException e) {
            log.error("❌ Mail delivery failed for {}: {}", email, e.getMessage());
            // OTP is stored even if email fails
            res.put("success", true);
            res.put("message", "OTP generated (email delivery may be delayed).");
        } catch (Exception e) {
    ...
    public Map<String, Object> verifyOTP(String email, String otp) {
        ...
        try {
            ...
            // Correct — consume OTP immediately
            otpStore.remove(email);
    ...
    public Map<String, Object> resendOTP(String email) {
        ...
        try {
            ...
            log.info("📧 OTP resent to: {} - OTP: {}", email, newOtp);
            res.put("success",       true);
            res.put("message",       "New OTP sent to " + email);
            res.put("expiryMinutes", OTP_EXPIRY_MINUTES);
            res.put("debug_otp", newOtp);
        } catch (MailSendException | MailAuthenticationException e) {
            log.error("❌ Resend mail failed for {}: {}", email, e.getMessage());
            res.put("success", true);
            res.put("message", "OTP regenerated (email delivery may be delayed).");
        } catch (Exception e) {
    ...
    private String generateSecureOTP() {
        // First digit is 1–9 to avoid leading-zero display issues in some email clients
