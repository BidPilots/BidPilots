...
public class GeMScrapingService {
	...
	// Single job guard - prevents two full scrapes from running at once
	...
	public ScrapingResult scrapeProductionMode() {
		log.info("=".repeat(80));
		log.info("🏭 PRODUCTION MODE SCRAPING - DEMAND-DRIVEN, STEALTH MODE ACTIVE");
		log.info("   Threads: {}, Rotating User Agents, Random Delays", THREAD_POOL_SIZE);
		log.info("   Only scraping states/cities demanded by at least one user filter");
		log.info("=".repeat(80));
	...
	// FIX: @Transactional removed - this method manages threads and does no DB work
	// itself.
	...
	private ScrapingResult scrapeInternal(int maxPages, String mode) {
		...
		try {
			...
			// ── DEMAND-DRIVEN: only fetch states with is_demanded = true ──────
			List<State> demandedStates = stateRepository.findDemandedStates();

			if (demandedStates.isEmpty()) {
				log.warn("⚠️  No demanded states found. Scraper will idle until users save filters.");
				log.warn("    Tip: once a user saves a filter with a state selected, that state");
				log.warn("    gets marked is_demanded = true and will be scraped next cycle.");
				result.setErrorMessage("No demanded states — nothing to scrape");
				return result;
			}

			log.info("📊 Demand-driven scope: {} demanded state(s) out of {} total", demandedStates.size(),
					stateRepository.count());

			// Shuffle states for anti-detection
			Collections.shuffle(demandedStates);

			ExecutorService stateExecutor = Executors.newFixedThreadPool(THREAD_POOL_SIZE);
			List<CompletableFuture<Void>> futures = new ArrayList<>();

			for (State state : demandedStates) {
				CompletableFuture<Void> future = CompletableFuture
						.runAsync(() -> processState(state, maxPages, ctx), stateExecutor).exceptionally(throwable -> {
			...
			CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).orTimeout(12, TimeUnit.HOURS).join();
			...
			log.info("=".repeat(80));
			log.info("✅ {} SCRAPING COMPLETED — DEMAND-DRIVEN", mode);
			log.info("   Demanded states scraped: {}", demandedStates.size());
			log.info("   New: {}, Updated: {}, Unchanged: {}, Reactivated: {}, Failed: {}", ctx.totalNewCount.get(),
					ctx.totalUpdatedCount.get(), ctx.totalUnchangedCount.get(), ctx.totalReactivatedCount.get(),
					ctx.totalFailedCount.get());
			log.info("   PDF Extraction - Success: {}, Failed: {}", ctx.totalPdfExtractedCount.get(),
					ctx.totalPdfFailedCount.get());
	...
	public ScrapingResult scrapeBidsByLocation(State state, City city, int maxPages, JobContext ctx) {
		...
		try {
			...
			log.info("   Max Pages: {} (will stop at last page)",
					maxPages == Integer.MAX_VALUE ? "ALL PAGES" : String.valueOf(maxPages));
			...
			clickElementNaturally(driver,
					By.xpath("//a[contains(@onclick, 'searchBid') and contains(@onclick, 'con')]"));
			sleepRandom(5000, 10000);
			...
			log.info("📊 Total pages for {}-{}: {} (will scrape all {})", state.getStateName(), city.getCityName(),
					totalPages, effectiveMax);
			...
			while (pageNumber < effectiveMax) {
				...
				if (!navigated) {
					log.error("❌ Could not navigate to page {}, stopping pagination for {}-{}", nextPage,
				...
				if (confirmedPage > 0 && confirmedPage != nextPage) {
					log.warn("⚠️ Expected page {} but pagination shows {} — continuing anyway", nextPage,
							confirmedPage);
				}
			...
			log.info("   Pages Scraped: {}, New: {}, Updated: {}, Unchanged: {}, Failed: {}", pageNumber,
	...
	private void processState(State state, int maxPages, JobContext ctx) {
		...
		log.info("[{}] Starting demanded state: {}", threadName, state.getStateName());

		try {
			// ── DEMAND-DRIVEN: only demanded cities for this state ─────────────
			List<City> demandedCities = cityRepository.findDemandedCitiesByState(state.getId());

			if (demandedCities.isEmpty()) {
				// State is demanded but no specific cities — scrape state level only
				log.info("[{}] {} has no demanded cities — scraping at state level (no city filter)", threadName,
						state.getStateName());
				scrapeBidsByLocation(state, createStateLevelPlaceholderCity(state), maxPages, ctx);
				return;
			}

			log.info("[{}] {} → {} demanded city(ies): {}", threadName, state.getStateName(), demandedCities.size(),
					demandedCities.stream().map(City::getCityName).toList());

			// Shuffle for anti-detection
			List<City> shuffled = new ArrayList<>(demandedCities);
			Collections.shuffle(shuffled);

			for (City city : shuffled) {
				try {
					log.info("[{}] Processing demanded city: {}-{}", threadName, state.getStateName(),
							city.getCityName());
					scrapeBidsByLocation(state, city, maxPages, ctx);
					sleepRandom(BASE_DELAY_MS, BASE_DELAY_MS + RANDOM_DELAY_RANGE);
				} catch (Exception e) {
					log.error("[{}] Error processing city {}: {}", threadName, city.getCityName(), e.getMessage());
					ctx.totalFailedCount.incrementAndGet();
				}
			}
	...
	/**
	 * Process a single bid - save AND extract PDF SYNCHRONOUSLY. REQUIRES_NEW gives
	 * each bid its own transaction so failures don't roll back the whole page.
	 */
	 ...
	 */
	private void scrapeOnePage(WebDriver driver, State state, City city, int pageNumber, int totalPages,
			JobContext ctx) {
		log.info("📄 Scraping page {}/{} - {}-{}", pageNumber, totalPages, state.getStateName(), city.getCityName());
	...
	/**
	 * Read the total number of pages from the pagination bar on the first page. GeM
	 * portal shows: « Prev 1 2 3 ... 66 Next » Returns 1 if pagination is absent
	 * (single page or no results).
	 */
	private int getTotalPages(WebDriver driver) {
		try {
			...
			while (m.find()) {
				...
				try {
					maxPage = Math.max(maxPage, Integer.parseInt(val));
				} catch (Exception ignored) {
				}
			}
			if (maxPage > 0) {
	...
	/**
	 * Navigate to a specific page number using the GeM portal's JS pagination. GeM
	 * renders results via goToPage(N) or equivalent – we invoke it directly via
	 * JavaScript so empty-result pages never block navigation.
	 *
	...
	private boolean navigateToPage(WebDriver driver, int targetPage, int currentPage) {
		try {
			...
			try {
				...
				if (nextBtns.isEmpty())
					nextBtns = driver.findElements(By.linkText("Next"));
				if (nextBtns.isEmpty())
					nextBtns = driver.findElements(By.xpath("//a[contains(text(),'Next')]"));
				...
				for (WebElement btn : nextBtns) {
					...
					if (!disabled && btn.isDisplayed()) {
						((JavascriptExecutor) driver)
								.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", btn);
						sleepRandom(500, 1500);
			...
			try {
				...
				String jsFunc = null;
				if (pageSource.contains("goToPage("))
					jsFunc = "goToPage";
				else if (pageSource.contains("changePage("))
					jsFunc = "changePage";
				else if (pageSource.contains("loadPage("))
					jsFunc = "loadPage";
				else if (pageSource.contains("gotopage("))
					jsFunc = "gotopage";
	...
	/**
	 * Verify which page we are currently on by reading the pagination bar. Returns
	...
	private boolean noResultsFound(WebDriver driver) {
		try {
			...
			return pageSource.contains("no bids found") || pageSource.contains("no records found")
					|| pageSource.contains("no data available") || pageSource.contains("no result found");
		} catch (Exception e) {
			return false;
		}
	}
	...
	private City createStateLevelPlaceholderCity(State state) {
		City placeholder = new City();
		placeholder.setId(null);
		placeholder.setCityName("(all cities)");
		placeholder.setState(state);
		placeholder.setIsActive(true);
		placeholder.setIsDemanded(false); // it's a virtual placeholder, not a real demanded city
		return placeholder;
	}
	...
	private boolean hasBidChanged(Bid existing, Bid scraped) {
		if (!Objects.equals(existing.getRaNumber(), scraped.getRaNumber()))
			return true;
		if (!Objects.equals(existing.getBidDocumentUrl(), scraped.getBidDocumentUrl()))
			return true;
		if (!Objects.equals(existing.getRaDocumentUrl(), scraped.getRaDocumentUrl()))
			return true;
		if (!Objects.equals(existing.getItems(), scraped.getItems()))
			return true;
		if (!Objects.equals(existing.getDataContent(), scraped.getDataContent()))
			return true;
		if (!Objects.equals(existing.getQuantity(), scraped.getQuantity()))
			return true;
		if (!Objects.equals(existing.getDepartment(), scraped.getDepartment()))
			return true;
		if (!Objects.equals(existing.getMinistry(), scraped.getMinistry()))
			return true;
		if (!Objects.equals(existing.getBidType(), scraped.getBidType()))
			return true;

		if (existing.getState() != null && scraped.getState() != null) {
			if (!Objects.equals(existing.getState().getId(), scraped.getState().getId()))
				return true;
		...
		if (existing.getConsigneeCity() != null && scraped.getConsigneeCity() != null) {
			if (!Objects.equals(existing.getConsigneeCity().getId(), scraped.getConsigneeCity().getId()))
				return true;
	...
	private boolean updateExistingBid(Bid existingBid, Bid scrapedBid) {
		...
		if (scrapedBid.getRaNumber() != null && !scrapedBid.getRaNumber().equals(existingBid.getRaNumber())) {
			existingBid.setRaNumber(scrapedBid.getRaNumber());
			updated = true;
		}
		if (scrapedBid.getBidDocumentUrl() != null
				&& !scrapedBid.getBidDocumentUrl().equals(existingBid.getBidDocumentUrl())) {
			existingBid.setBidDocumentUrl(scrapedBid.getBidDocumentUrl());
			updated = true;
		}
		if (scrapedBid.getRaDocumentUrl() != null
				&& !scrapedBid.getRaDocumentUrl().equals(existingBid.getRaDocumentUrl())) {
			existingBid.setRaDocumentUrl(scrapedBid.getRaDocumentUrl());
			updated = true;
		}
		if (scrapedBid.getItems() != null && !scrapedBid.getItems().isEmpty()
				&& !scrapedBid.getItems().equals(existingBid.getItems())) {
			existingBid.setItems(scrapedBid.getItems());
			updated = true;
		}
		if (scrapedBid.getDataContent() != null && !scrapedBid.getDataContent().isEmpty()
				&& !scrapedBid.getDataContent().equals(existingBid.getDataContent())) {
			existingBid.setDataContent(scrapedBid.getDataContent());
			updated = true;
		}
		if (scrapedBid.getQuantity() != null && !scrapedBid.getQuantity().equals(existingBid.getQuantity())) {
			existingBid.setQuantity(scrapedBid.getQuantity());
			updated = true;
		}
		if (scrapedBid.getDepartment() != null && !scrapedBid.getDepartment().equals(existingBid.getDepartment())) {
			existingBid.setDepartment(scrapedBid.getDepartment());
			updated = true;
		}
		if (scrapedBid.getMinistry() != null && !scrapedBid.getMinistry().equals(existingBid.getMinistry())) {
			existingBid.setMinistry(scrapedBid.getMinistry());
			updated = true;
		}
		if (scrapedBid.getBidStartDate() != null
				&& !scrapedBid.getBidStartDate().equals(existingBid.getBidStartDate())) {
			existingBid.setBidStartDate(scrapedBid.getBidStartDate());
			updated = true;
		}
	...
	private void startProgressLogger(String mode, JobContext ctx) {
		...
		progressLogger.scheduleAtFixedRate(() -> log.info(
				"[{}] Progress - New: {}, Updated: {}, Unchanged: {}, Reactivated: {}, Failed: {}, PDF Success: {}, PDF Failed: {}",
				mode, ctx.totalNewCount.get(), ctx.totalUpdatedCount.get(), ctx.totalUnchangedCount.get(),
				ctx.totalReactivatedCount.get(), ctx.totalFailedCount.get(), ctx.totalPdfExtractedCount.get(),
				ctx.totalPdfFailedCount.get()), 1, 5, TimeUnit.MINUTES);
	}
	...
	public static class ScrapingResult {
		...
		public void setTotalNewCount(int v) {
			this.totalNewCount = v;
		}
		...
		public void setTotalUpdatedCount(int v) {
			this.totalUpdatedCount = v;
		}
		...
		public void setTotalUnchangedCount(int v) {
			this.totalUnchangedCount = v;
		}
		...
		public void setTotalFailedCount(int v) {
			this.totalFailedCount = v;
		}

		public int getTotalReactivatedCount() {
			return totalReactivatedCount;
		}

		public void setTotalReactivatedCount(int v) {
			this.totalReactivatedCount = v;
		}

		public int getTotalPdfExtracted() {
			return totalPdfExtracted;
		}

		public void setTotalPdfExtracted(int v) {
			this.totalPdfExtracted = v;
		}

		public int getTotalPdfFailed() {
			return totalPdfFailed;
		}

		public void setTotalPdfFailed(int v) {
			this.totalPdfFailed = v;
		}
		...
		public void setTimeTakenSeconds(long v) {
			this.timeTakenSeconds = v;
		}

		public int getPagesScraped() {
			return pagesScraped;
		}

		public void setPagesScraped(int v) {
			this.pagesScraped = v;
		}
